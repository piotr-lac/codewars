...
# "weight" 1 so in the list 100 will come before 99. Given a string with the weights 
# of FFC members in normal order can you give this string ordered by "weights" of these numbers?

# Example:

# "56 65 74 100 99 68 86 180 90" ordered by numbers 
# weights becomes: "100 180 90 56 65 74 68 86 99"

# When two numbers have the same "weight", 
# let us class them as if they were strings and not numbers: 100 is before 180 
# because its "weight" (1) is less than the one of 180 (9) and 180 is before 90 since, 
# having the same "weight" (9) it comes before as a string.

# All numbers in the list are positive numbers 
...
def order_weight(strng)
    ...
    arr=strng.split
    (arr.length).times {|i| h[arr[i].to_i]=arr[i]}
		
    h.each_value do |v|
    	suma_znakow=0
    	 v.each_char {|c| suma_znakow = suma_znakow + c.to_i}
    	 b << suma_znakow
    end

    h.transform_values!.with_index {|v, i| v = b[i]}
    wynik = h.sort_by {|k,v| v}.to_h
    wynik.each_key {|k| koniec << k}
